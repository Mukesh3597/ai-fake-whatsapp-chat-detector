...
def predict_image(file_path: str):
    ...
    # model output could be sigmoid (1 value) or softmax (2 values)
    ...
    if y.shape[-1] == 1:
        ...
        # assume [real, fake] or similar
        probs = y[0].astype(float)
        idx = int(np.argmax(probs))
        confidence = round(float(probs[idx]) * 100, 2)

        # class mapping (safe default)
        # If your training order differs, swap these two
